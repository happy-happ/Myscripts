...
#赢一加新品手机
25 9 * 5,6 * node /scripts/jd_oneplus.js >> /scripts/logs/jd_oneplus.log 2>&1
#飞利浦电视成长记
15 9 * 5,6 * node /scripts/jd_flp.js >> /scripts/logs/jd_flp.log 2>&1
#京东极速版邀请好友
...
##############长期活动##############
#柠檬赚京豆步数换京豆
0 23 * * * node /scripts/jd_wxbs.js >> /scripts/logs/jd_wxbs.log 2>&1
# 签到
...
# 京喜财富岛提现
0 0 0 * * *  node /scripts/jx_cfdtx.js >> /scripts/logs/jx_cfdtx.log 2>&1
# 京东极速版
...
#试用（最新版）
3 2 * * *  node /scripts/try.js >> /scripts/logs/try.log 2>&1
